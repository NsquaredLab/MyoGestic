...
Everything generic lives in `myogestic.remote`: the target, the gRPC clients, the
`~myogestic.remote.InterfaceSpec` dataclass and the wire contract they all speak. VHI
is one remote target among however many, and imports that half like any other consumer.
...
- `pose` — the channel layout of VHI's *recorded* nine-float pose. A layout, not an
  interface: the live control path asks VHI where its controls are and never
  consults a table.
"""
