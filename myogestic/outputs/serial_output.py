...
class SerialOutput(Output):
    ...
    Args:
        port: Serial port path. Examples: ``"/dev/cu.usbmodem1101"``
            on macOS, ``"/dev/ttyACM0"`` on Linux, ``"COM3"`` on
            Windows.
        baud: Baud rate. Default 115200 - matches typical Arduino
            sketches; tune to your firmware.
        hz: Send rate of the daemon thread (Hz). Default 10. Keep this
            low: serial UART has a hard ceiling and queue-induced
            latency stacks fast.
        fmt: `struct` format character per value. ``"f"`` for float32
