...
from myogestic.gui.widgets.visual_interfaces.virtual_cursor_interface.ui import (
    Ui_SetupVirtualCursorInterface
)
...
class VirtualCursorInterface_SetupInterface(SetupInterfaceTemplate):
    ...
    incoming_message_signal = Signal(np.ndarray, str)
    ...
    def __init__(self, main_window, name="VirtualCursorInterface"):
        ...
        self._cursor_process.started.connect(
        ...
        self._cursor_process.finished.connect(
            lambda: self._main_window.toggle_selected_visual_interface(self.name)
        )
    ...
    def initialize_ui_logic(self):
        ...
        self._toggle_virtual_cursor_interface__push_button: QPushButton = (
            self.ui.toggleVirtualCursorInterfacePushButton
        )
        self._toggle_virtual_cursor_interface__push_button.clicked.connect(
            self.toggle_virtual_cursor_interface
        )
        self._virtual_cursor_interface__status_widget: QWidget = (
            self.ui.virtualCursorInterfaceStatusWidget
        )

        self._virtual_cursor_interface__status_widget.setStyleSheet(
    ...
    def start_interface(self):
        ...
        if not self._use_external_virtual_cursor_interface__check_box.isChecked():
            ...
            if not os.path.exists(main_cursor_path):
                self._main_window.logger.print(
                    f"Error: main_cursor.py not found at expected path: {main_cursor_path}",
            ...
            self._cursor_process.start(python_executable, [main_cursor_path])
    ...
    def toggle_streaming(self) -> None:
        ...
        if self._toggle_virtual_cursor_interface__push_button.isChecked():
            ...
            self._predicted_cursor__udp_socket.bind(
                QHostAddress(SOCKET_IP), VCI_PREDICTION__UDP_PORT
            )
    ...
    def write_message(self, message: QByteArray) -> None:
        ...
        if self._is_connected and (
            ...
            output_bytes = self._streaming__udp_socket.writeDatagram(
                message, QHostAddress(SOCKET_IP), VCI__UDP_PORT
            )
            ...
            while self._streaming__udp_socket.hasPendingDatagrams():
                ...
                try:
                    ...
                    try:
                        ...
                        self.incoming_message_signal.emit(coord_array, task_label)
