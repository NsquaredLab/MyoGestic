...
class TrialPreview:
    ...
    def __init__(
        ...
        *,
        ...
        widget_id
            ...
            two ``TrialPreview`` instances in the same frame don't collide).
        data_layout
            ``"channels_first"`` (default) treats data as
            ``(n_channels, n_samples)``; ``"samples_first"`` as
    ...
    def ui(self, data: np.ndarray, fs: float) -> None:
        """Render stacked multi-channel waveform with optional band overlay.
        ...
        try:
            ...
            if self._data_layout == "samples_first":
            ...
            if self._display_filter != "none":
                arr = apply_display_filter(
                    arr.T.astype(np.float32, copy=False), self._display_filter, fs
                ).T.astype(np.float64)

            if self._as_window:
                imgui.set_next_window_size(imgui.ImVec2(960, 480), imgui.Cond_.first_use_ever)
                title_id = (self._title or "Trial Preview") + f"###trial_preview_{self._widget_id}"
            ...
            # range with 20 % padding (matches signal_viewer's auto behaviour).
            if self._scale_mode == "manual":
                lane = max(float(self._y_range[1]) - float(self._y_range[0]), 1e-9)
            else:
                ...
                if finite.size:
                    ...
                    lane = 1.0
            # The y-axis extent always spans all channels with a half-lane pad
            # on top and bottom — without this, manual mode would show only
            # channel 0 and clip channels 1..n_ch-1 below the visible y range.
            y_hi = lane * 0.6
            ...
            if implot.begin_plot(
                ...
                imgui.ImVec2(self._size[0], self._size[1]),
                flags=flags,
            ):
                ...
                if self._band is not None:
                    bc = self._band_color if self._band_color is not None else (0.4, 0.7, 1.0, 1.0)
                    spec = implot.Spec()
                    spec.fill_color = imgui.ImVec4(bc[0], bc[1], bc[2], 1.0)
                    spec.fill_alpha = bc[3] if len(bc) >= 4 else 0.22
                    implot.plot_shaded(
                        f"band##{self._widget_id}",
                        np.array(
                            [float(self._band[0]), float(self._band[1])],
                            dtype=np.float64,
                        ),
                ...
                for ch in range(n_ch):
                    ys = np.ascontiguousarray(arr[ch] * self._gain - ch * lane, dtype=np.float64)
                    label = (
                        self._channel_names[ch]
